...
class LineRenderer<D> extends BaseCartesianRenderer<D> {
  ...
  /// missing/null data.
  AccessorFn<num?> _createStackedMeasureOffsetFunction(MutableSeries<D> series, Map<D, num?> curOffsets, Map<D, num> nextOffsets) {
    final domainFn = series.domainFn;
    final measureFn = series.measureFn;
    ...
    seriesList.forEach((ImmutableSeries<D> series) {
      ...
      styleSegments.forEach((styleSegment) {
        ...
        if (animatingElements != null) {
          ...
          final lineAndArea = _createLineAndAreaElements(series, styleSegment as _LineRendererElement<D>, stackIndex > 0 ? previousInitialPointList[stackIndex - 1] : null, true);
          ...
          for (var index = 0; index < lineElementList.length; index++) {
            animatingLines.add(_AnimatedLine<D>(key: lineElementList[index].styleKey, overlaySeries: series.overlaySeries)..setNewTarget(lineElementList[index]));
          }
          ...
          if (config.includeArea) {
            ...
            for (var index = 0; index < areaElementList.length; index++) {
              animatingAreas.add(_AnimatedArea<D>(key: areaElementList[index].styleKey, overlaySeries: series.overlaySeries)..setNewTarget(areaElementList[index]));
            }
          }
          ...
          if (_hasMeasureBounds) {
            ...
            for (var index = 0; index < boundsElementList.length; index++) {
              animatingBounds.add(_AnimatedArea<D>(key: boundsElementList[index].styleKey, overlaySeries: series.overlaySeries)..setNewTarget(boundsElementList[index]));
            }
          }
        ...
        final lineAndArea = _createLineAndAreaElements(series, styleSegment as _LineRendererElement<D>, stackIndex > 0 ? previousPointList[stackIndex - 1] : null, false);
        final lineElementList = lineAndArea[0] as List<_LineRendererElement<D>>;
        final areaElementList = lineAndArea[1] as List<_AreaRendererElement<D>>;
        final allPointList = lineAndArea[2] as List<_DatumPoint<D>>;
        final boundsElementList = lineAndArea[3] as List<_AreaRendererElement<D>>;
        ...
        for (var index = 0; index < lineElementList.length; index++) {
          ...
          if (index >= animatingElements.lines.length) {
            animatingElements.lines.add(_AnimatedLine<D>(key: lineElement.styleKey, overlaySeries: series.overlaySeries));
          }
        ...
        if (config.includeArea) {
          for (var index = 0; index < areaElementList.length; index++) {
            ...
            if (index >= animatingElements.areas!.length) {
              animatingElements.areas!.add(_AnimatedArea<D>(key: areaElement.styleKey, overlaySeries: series.overlaySeries));
            }
        ...
        if (_hasMeasureBounds) {
          for (var index = 0; index < boundsElementList.length; index++) {
            ...
            if (index >= animatingElements.bounds!.length) {
              animatingElements.bounds!.add(_AnimatedArea<D>(key: boundElement.styleKey, overlaySeries: series.overlaySeries));
            }
  ...
  List<Object> _createLineAndAreaElements(ImmutableSeries<D> series, _LineRendererElement<D> styleSegment, List<_DatumPoint<D>>? previousPointList, bool initializeFromZero) {
  ...
  List<_DatumPoint<D>> _createAreaSegment(
      int start, int end, List<_DatumPoint<D>> pointList, List<_DatumPoint<D>>? previousPointList, ImmutableSeries<D> series, bool initializeFromZero) {
    final domainAxis = series.getAttr(domainAxisKey) as ImmutableAxis<D>;
    final domainFn = series.domainFn;
    final measureAxis = series.getAttr(measureAxisKey) as ImmutableAxis<num>;
  ...
  List<_DatumPoint<D>> _createBoundsSegment(List<_DatumPoint<D>> pointList, ImmutableSeries<D> series, bool initializeFromZero) {
    ...
    areaPointList.addAll(pointList.map((datumPoint) => _DatumPoint.from(datumPoint, datumPoint.x,
        initializeFromZero ? datumPoint.y : measureAxis.getLocation((series.measureUpperBoundFn!(datumPoint.index) ?? 0) + series.measureOffsetFn!(datumPoint.index)!))));

    // Add all points for lower bounds, in reverse order.
    areaPointList.addAll(pointList.reversed.map((datumPoint) => _DatumPoint.from(datumPoint, datumPoint.x,
        initializeFromZero ? datumPoint.y : measureAxis.getLocation((series.measureLowerBoundFn!(datumPoint.index) ?? 0) + series.measureOffsetFn!(datumPoint.index)!))));
    ...
    _seriesLineMap.forEach((String key, List<_AnimatedElements<D>> elements) {
      if (config.includeArea) {
        elements
            ...
            .forEach((area) {
          canvas.drawPolygon(clipBounds: _getClipBoundsForExtent(area.positionExtent), fill: area.areaColor ?? area.color, points: area.points.toPoints());
        });
      }
      ...
      if (_hasMeasureBounds) {
        elements
            ...
            .forEach((bound) {
          canvas.drawPolygon(clipBounds: _getClipBoundsForExtent(bound.positionExtent), fill: bound.areaColor ?? bound.color, points: bound.points.toPoints());
        });
      }
  ...
  Rectangle<num> _getClipBoundsForExtent(_Range<num> extent) {
    ...
    return Rectangle<num>(left, drawBounds.top - drawBoundTopExtensionPx, right - left, drawBounds.height + drawBoundTopExtensionPx + drawBoundBottomExtensionPx);
  }

  bool get isRtl => _chart?.context.isRtl ?? false;

  _DatumPoint<D> _getPoint(
      dynamic datum, D? domainValue, ImmutableSeries<D> series, ImmutableAxis<D> domainAxis, num? measureValue, num? measureOffsetValue, ImmutableAxis<num> measureAxis,
      {int? index}) {
    ...
    return _DatumPoint<D>(datum: datum, domain: domainValue, series: series, x: domainPosition, y: measurePosition, index: index);
  }
  ...
  List<DatumDetails<D>> getNearestDatumDetailPerSeries(
    ...
    for (final seriesSegments in _seriesLineMap.values) {
      ...
      for (final segment in seriesSegments) {
        ...
        if (config.includeArea && !isFirstSeriesAbovePoint && nearestPoint != null && _isPointBelowSeries(chartPoint, nearestPoint, segment.allPoints)) {
  ...
  /// [chartPoint].
  bool _isPointBelowSeries(Point<double> chartPoint, _DatumPoint<D> nearestPoint, List<_DatumPoint<D>> allPointsForSeries) {
...
class _DatumPoint<D> extends NullablePoint {
  ...
  factory _DatumPoint.from(_DatumPoint<D> other, [double? x, double? y]) {
    return _DatumPoint<D>(datum: other.datum, domain: other.domain, series: other.series, index: other.index, x: x ?? other.x, y: y ?? other.y);
  }
}
...
class _LineRendererElement<D> {
  ...
  void updateAnimationPercent(_LineRendererElement<D> previous, _LineRendererElement<D> target, double animationPercent) {
...
class _AreaRendererElement<D> {
  ...
  void updateAnimationPercent(_AreaRendererElement<D> previous, _AreaRendererElement<D> target, double animationPercent) {
    late _DatumPoint<D> lastPoint;
